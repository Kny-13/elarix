...
def mascot_img(expr):
    ...
    # Try multiple possible folders where images might be
    script_folder = os.path.dirname(os.path.abspath(__file__))
    cwd_folder = os.getcwd()
    search_folders = list(dict.fromkeys([script_folder, cwd_folder]))  # deduplicated
    ...
    for folder in search_folders:
        for candidate in candidates:
            ...
            if os.path.exists(full_path):
                print(f"[MASCOT] Serving: {full_path}")
                return send_from_directory(folder, candidate)

    print(f"[MASCOT] NOT FOUND: {name} — looked in: {search_folders}")
    return "", 404

@app.route("/mascot-debug")
def mascot_debug():
    """Visit /mascot-debug in browser to see what files are found"""
    folder = os.path.dirname(os.path.abspath(__file__))
    files = [f for f in os.listdir(folder) if "mascot" in f.lower()]
    return "<br>".join([
        f"<b>Script folder:</b> {folder}",
        f"<b>Mascot files found:</b> {files if files else 'NONE'}",
        "<br><b>Expected files:</b> mascot_normal.png, mascot_happy.png, mascot_angry.png, mascot_shocked.png, mascot_thinking.png, mascot_sad.png"
    ])
