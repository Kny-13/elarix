...
function popOne(btn) {
  ...
  .catch(function(){
    // if API fails, still let user pop visually and exit after 5
...
function forceExitBubbles() {
  // Emergency exit — resets bubble state on server and closes overlay
  fetch('/api/bubble/reset', {method:'POST', headers:{'Content-Type':'application/json'}})
  .then(function(){ 
    document.getElementById('bub-ov').classList.remove('show');
    busy = false;
...
// On page load, check if bubble_mode is active in session and show overlay
window.addEventListener('load', function(){
  fetch('/api/session/state')
  .then(function(r){ return r.json(); })
  .then(function(d){
    if (d.bubble_mode) showBubbles();
  });
});
